...
// upload.php - maneja uploads desde el frontend
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($file_tmp, $target_path)) {
        // Actualizar base de datos según el documento (si está disponible)
        $resultado = null;
        if (function_exists('actualizarEstadoDocumento')) {
            try {
                $resultado = actualizarEstadoDocumento($documento_id, $num_cuenta);
            } catch (Throwable $e) {
                $resultado = ['error' => 'Excepción en actualizarEstadoDocumento: ' . $e->getMessage()];
            }
        } else {
            $resultado = ['warning' => 'Función actualizarEstadoDocumento no disponible'];
        }

        // Registrar en log (archivo local dentro de uploads)
